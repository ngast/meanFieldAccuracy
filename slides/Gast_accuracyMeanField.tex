...
\begin{frame}{How accurate is mean-field approximation?}

  \begin{center}
    
    \mpage{.45}{
      \begin{exampleblock}{Theorem (Kurtz 70\dots \uncover<3->{Ying 16}) }
...
\begin{frame}{In practice, how accurate is mean-field approximation?}
  {Can we use the approximation for $N=1000$? $N=100$? $N=10$?}
  ...
  \begin{table}[ht]
    ...
    \begin{tabular}{|c|c|c|c|c|c|}
      ...
      \only<1>{&&&&}\only<2->{Error of mean-field&      0.4513	&0.0404
                                        &0.0040 &0}\\%
      \noalign{}\hline\noalign{}
    \end{tabular}    
  ...
  \uncover<3->{
    Contributions : 
    \begin{enumerate}
    \item We show that under very general conditions, the error is in
      $O(1/N)$
    \item We show that for that, the drift needs to be
...
\section{The Kurtz's Population Model : Classical Convergence Results}
...
% For Lipschitz-continuous, is the convergence rate
%       $O(1/\sqrt{N})$?
    \end{itemize}
